...
const PermissionEditModal: React.FC<EditModalProps> = (props) => {
  ...
  const [dataSource, setDataSource] = useState<ExtOrgPos[]>(
    mergeProps.values.sysEmpInfo?.extOrgPos || [],
  );
  useEffect(() => {
  ...
  const onFinish = async (value: any) => {
    if (mergeProps.action == 'edit') {
      value.sysEmpInfo.posIdList = value.positionsCode.map((src: number) => {
        return src;
      });
  ...
  return (
    <ModalForm
      ...
      initialValues={{
        ...mergeProps.values,
        ...{ positionsCode: mergeProps.values.sysEmpInfo?.positions.map((src) => src.posId) },
      }}
