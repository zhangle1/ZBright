...
import { applicationPage, deleteApplication, setAsDefault } from './service';
...
const ApplicationList: React.FC = () => {
  ...
  const pageEvent = {
    addApplication: () => {
      setFieldModalProps((prevState) =>
        merge(defalutFieldPropsValue, {
          title: '新增',
          visiable: true,
          action: 'add',
        } as FieldModalProps),
    ...
    setActiveDefault: async (entity: any) => {
      await setAsDefault({ id: entity.id });
      actionRef.current?.reload()
    },
    deleteApplication: async(entity: any)=>{
      await deleteApplication({id: entity.id})
      actionRef.current?.reload()
    }
  };
        ...
        <ActionButtonContainer
          scheme={[
            ...
            },
            {
              ...
              popTitle: '是否要设置默认菜单',
              onConfirm: () => {
                pageEvent.setActiveDefault(entity);
              },
              onClick: () => {},
              visual: entity.active != 'Y',
            },
          ]}
        />
      ),
    ...
    <GridContent>
      ...
      <ApplicationFieldModal
        ...
        onFinish={() => {
          actionRef.current?.reload();
          setFieldModalProps(
            merge(defalutFieldPropsValue, { visiable: false, action: '' } as FieldModalProps),
          );
        }}
        onCancel={(b) => {
          if (!b) {
